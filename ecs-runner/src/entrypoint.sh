...
# If we don't have a RUNNER_TOKEN, we should try to get the secret from Secrets Manager
if [[ -z $RUNNER_TOKEN ]]; then
    GITHUB_ACCESS_TOKEN=`aws secretsmanager get-secret-value --secret-id githubtoken | jq -r '.SecretString'`
fi
